...
def clean_object_pointcloud( im_confs, other_confs, mask_bool, K, cams, depthmaps, all_pts3d, 
                      tol=0.001, bad_conf=0, dbg=()):
    """ Method: 
    1) express all 3d points in each camera coordinate frame
    2) if they're in front of a depthmap --> then lower their confidence
    ...
    for i, pts3d in enumerate(all_pts3d):
        for j in range(len(all_pts3d)):
            if i == j: continue
            print("Res shape: ", res[i].shape)
            # project 3dpts in other view
            proj = geotrf(cams[j], pts3d)
            proj_depth = proj[:, 2]     # z-coord of 3D points 
            print("Proj depth shape: ", proj_depth.shape)
            u,v = geotrf(K[j], proj, norm=1, ncol=2).round().long().unbind(-1) # 2D object points in the j-th view

            print("U shape: ", u.shape)
            print("V shape: ", v.shape)
            # check which points are actually in the visible cone of other PoV
            H, W = other_confs[j].shape
            valid_mask = (proj_depth > 0) & (0 <= u) & (u < W) & (0 <= v) & (v < H)
            print("Valid mask shape: ", valid_mask.shape)

            # Map 2D indices back to 1D indices for checking within image
            linear_idx = v[valid_mask] * W + u[valid_mask]

            # Check if points fall within the mask in the other view
            print("Mask bool shape: ", mask_bool[j].shape)
            within_image = mask_bool[j].flatten()[linear_idx]
            valid_mask[valid_mask.clone()] &= within_image  # Update the mask to include only points within the mask
            
            print("Valid mask shape: ", valid_mask.shape)
            print("Linear idx shape: ", linear_idx.shape)
            print("Within mask shape: ", within_image.shape)
            print("Valid mask: ", valid_mask)
            true_count_valid_mask = valid_mask.sum()
            print("True count valid mask: ", true_count_valid_mask)
            print("within image: ", within_image)
            true_count_within_image = within_image.sum()
            print("True count within image: ", true_count_within_image)
            # Check for bad points: Points within the mask but with lower confidence
            if valid_mask.any():
                bad_points = (
                    (proj_depth[valid_mask] < (1 - tol) * depthmaps[j][linear_idx[within_image]]) &
                    (res[i][valid_mask] < other_confs[j].flatten()[linear_idx[within_image]])
                )

                # Reduce confidence for bad points
                res[i][valid_mask] = res[i][valid_mask].clone()
                res[i][valid_mask][bad_points] = bad_conf
